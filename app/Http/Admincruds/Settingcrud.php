...
class Settingcrud{
    ...
    public function sliderpublish($id){
        ...
        $publish->status = 0;
        $publish->save();
         return redirect()->route('admin.slider')->with('success','Publish successfully');
    }
    public function sliderunpublish($id){
        $unpublish =  Slider::find($id);
        $unpublish->status = 1;
        $unpublish->save();
        return redirect()->route('admin.slider')->with('success','Unpublish successfully');
    }
    ...
    public function white()    {
        ...
        return view('admin.white.index', compact('userData'));
    }
    public function whitecreate()    {  
        ...
        ->get()
        ->unique('ip');
        return view('admin.white.create', compact('users','distinks'));
    }
    ...
    public function whitestore( $request)    {
        ...
        if($request->isMethod('post')){
            ...
            if(empty($whits[0]->user_id)){      // for uniqe user_id login
                ...
                return redirect()->route('admin.white')->with('success','IP added white listed');
            }else{
                return redirect()->route('admin.white')->with('error','Allredy added');
            }
        }
    ...
    public function whiteedit($id)    {
        ...
        return view('admin.white.edit', compact('white','users'));
    }
    public function whiteupdate($request)    {
        ...
        $white->save();    
        return redirect()->route('admin.white')->with('success','Update successfully');
    }
    public function whitedestroy($id)    {
        $destroyID = Whitelist::findOrFail($id);
        $destroyID->delete();
        return redirect()->route('admin.white')->with('success','IP and User Deleted');
    ...
    public function black(){
        $userID = Auth::id();
        $userData = User::where('id', $userID)->get(); 
        return view('admin.black.index', compact('userData'));
    }
    public function blackcreate(){
        $users = DB::table('users')
        ->select('*')
        ->get();
        return view('admin.black.create', compact('users'));
    }
    ...
    public function blackstore($request)    {
        ...
        if($request->isMethod('post')){
            ...
            Blacklist::create([
                ...
                return redirect()->route('admin.black')->with('success','IP added black listed');
            }else{
                return redirect()->route('admin.black')->with('error','Allredy added');
            }
        }
    }
    public function blackedit($id)  {
        ...
        $users = DB::table('users')
            ->select('*')
            ->get(); 
        return view('admin.black.edit', compact('black','users'));
    }
    ...
    public function blackupdate( $request)    {
        ...
        $black->save();    
        return redirect()->route('admin.black')->with('success','Update successfully');
    }
    public function blackdestroy($id){
        $destroyID = Blacklist::findOrFail($id);
        $destroyID->delete();
        return redirect()->route('admin.black')->with('success','IP and User Deleted');
    }
    // ============================CSV===============================
    ...
    public function articalindex()   {
        ...
        return view('admin.artical.index', compact('categories'));
    }
