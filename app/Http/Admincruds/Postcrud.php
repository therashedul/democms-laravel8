...
class Postcrud{    public function postindex($request)
    {
        ...
        $categories = DB::table('posts')
            ...
            ->get();
        return view('admin.post.index', compact('posts', 'userData', 'categories'));
    }
    ...
    public function postshow($slug)
    {
        ...
        */
        ...
        // print_r($postmeta);
        // die();
        return view('admin.post.show', compact('post', 'categories', 'postmeta'));
    ...
    public function poststore($request)
    {
        ...
        if ($request->method() == 'POST') {
            ...
            $postdate->publish_at = $request->input('publish_at')?$request->input('publish_at'): $datetime;
    ...
    public function postedit($id)
    {
        ...
        return view('admin.post.edit', compact('post', 'categories', 'postmeta'));
    }
    ...
    public function postupdate($request)
    {
        ...
        $postdate->save();
        return redirect()->route('admin.post')->with('success', 'Post  Update successfully');
    }
        ...
        if ($request->ajax()) {
            ...
            if (count($posts) > 0) {
                foreach ($posts as $key => $post) {
                    ...
                    $output .= '<a href=' . route('admin.post.show', $post->id) . ' class="btn btn-sm  btn-success"><i class="fa fa-eye"
                                                    aria-hidden="true"></i></a>';

                    if ($post->status == 1) {
                        $output .= '<a href=' . route('admin.post.publish', $post->id) . ' class="btn btn-sm btn-info"><i class="fa fa-arrow-circle-up"
                                                        aria-hidden="true"></i> </a>';
                    } else {
                        $output .= '<a href=' . route('admin.post.unpublish', $post->id) . ' class="btn btn-sm btn-warning"> <i class="fa fa-arrow-circle-down " aria-hidden="true"></i> </a>';
                    }
                    $output .= '<a href=' . route('admin.post.delete', $post->id) . ' class="btn btn-sm  btn-danger"><i class="fa fa-trash"
                                                    aria-hidden="true"></i> </a>';
                    $output .= '</td></tr>';
                }
    ...
    //        ->get();
    //        $post = $post[0];
    //     return view('admin.post.single-post', compact('post'));
    ...
    public function postarchive()
    {
        ...
        return view('admin.post.archive', compact('posts'));
    }
    public function postarchivereturn($id)
    {
        ...
        return redirect()->route('admin.post')->with('success', 'Post Reset Succesfully');
    }
    ...
    public function commentarchive()
    {
        ...
        return view('admin.comment.archivecomment', compact('comment'));
    }
    public function commentreturn($id)
    {
        Comment::withTrashed()->find($id)->restore();
        return redirect()->route('admin.comments')->with('success', 'Comment Reset Succesfully');
    }
